...
class RickerNetLayer1D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        super(RickerNetLayer1D, self).build(input_shape)  # Be sure to call this at the end
    ...
    def call(self, input_tensor, **kwargs):
        ...
        for i in range(self.fnum):
            temp = ricker(self.fbandwidth[i] * self.fs, self.t)
            temp = temp * window
...
def ricker(band, t):
    ...
    sigma = 2/(math.pi*band)
    ricker = (2/((tf.math.sqrt(3*sigma)*(math.pi**0.25))))*(1-(t**2/sigma**2))*tf.math.exp(-t**2 / (2 * sigma**2))
    ricker = ricker/tf.reduce_max(ricker)
    return ricker