...
from complex_valued_layers import sincnet_layer_ri as sinc  # or rename if needed
from complex_valued_layers import gaussian_layer_ri as gauss
from complex_valued_layers import gammatone_layer_ri as gamma
from complex_valued_layers import ricker_layer_ri as ricker
...
def complex_plf(input,fn,fl,fs,max_pool):
    x = sinc.SincNetLayer1D(fn, fl, fs)(input)
    # x = gauss.GaussNetLayer1D(fn, fl, fs)(input)
    # x = gamma.GammatoneNetLayer1D(fn, fl, fs)(input)
    # x = ricker.RickerNetLayer1D(fn, fl, fs)(input)
    x = pool.ComplexMaxPooling1D(max_pool, max_pool)(x)
